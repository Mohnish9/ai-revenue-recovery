...
export async function sendWhatsAppMessage(params: {
  ...
  if (!accountSid || !authToken) {
    return {
      ...
      status: "SIMULATED",
      deliveryLabel: "WhatsApp Simulated (No Twilio Credentials Configured)",
  ...
  if (!targetPhone) {
    return {
      ...
      deliveryMode: "FAILED",
      status: "FAILED",
      deliveryLabel: "WhatsApp Failed (Missing Phone Number)",
      isRealDispatch: true,
  ...
  try {
    const e164 = normalizeToE164(targetPhone);
    ...
    console.info(`[WhatsApp Dispatch] Sending to ${toWhatsApp} via ${fromWhatsApp} (Demo Contact: ${shouldUseTestContact})`);
    ...
    // If sandbox sender retry is needed (e.g., from number format mismatch)
    if (
      !response.ok &&
      ...
      if (contentSid) {
        ...
        retryFormData.append("Body", messageText);
      }
    ...
    // If Twilio Sandbox returns 21654 (Outbound WhatsApp Sandbox messages require pre-approved Content Template or active 24h session)
    // Attempt automatic retry with standard pre-approved Twilio WhatsApp Sandbox Content Templates
    if (!response.ok && data.code === 21654 && !contentSid) {
      const sandboxTemplates = [
        "HXb5b62575e6e4ff6129ad7c8efe1f983e",
        "HX229f5a04fd0510ce1b071852155d3e75",
      ];

      for (const tSid of sandboxTemplates) {
        console.info(`[WhatsApp Dispatch] Twilio 21654 encountered. Retrying with pre-approved Twilio Sandbox Content Template ${tSid}...`);
        const tFormData = new URLSearchParams();
        tFormData.append("To", toWhatsApp);
        tFormData.append("From", fromWhatsApp.includes("+14155238886") ? fromWhatsApp : "whatsapp:+14155238886");
        tFormData.append("ContentSid", tSid);
        tFormData.append("ContentVariables", JSON.stringify(templateVariables));
        ...
        const tResponse = await fetch(
          `https://api.twilio.com/2010-04-01/Accounts/${accountSid}/Messages.json`,
          {
            ...
            body: tFormData.toString(),
          }
        );
        const tData = await tResponse.json().catch(() => ({}));
        if (tResponse.ok && tData.sid) {
          response = tResponse;
          data = tData;
          break;
        }
    ...
    if (response.ok && data.sid) {
      console.info(`[WhatsApp Dispatch] REAL SUCCESS: Twilio SID ${data.sid}`);
      return {
        channel: "WHATSAPP",
        provider: "TWILIO",
        deliveryMode: "REAL",
        ...
        deliveryLabel: shouldUseTestContact
          ? `WhatsApp Sent via Twilio (Routed to Demo Contact: ${demoContact.verifiedPhone})`
          : `WhatsApp Sent via Twilio (SID: ${data.sid})`,
        isRealDispatch: true,
        destination: shouldUseTestContact ? `${rawDestination} (via ${demoContact.verifiedPhone})` : toWhatsApp,
        routedToTestContact: shouldUseTestContact,
        actualDestination: toWhatsApp,
      ...
      // Real Twilio rejection - strictly return FAILED (NEVER SENT) with crystal clear actionable diagnostic
      ...
      if (data.code === 572002) {
        ...
        errorMsg = "Twilio WhatsApp Error 63007: Recipient has not joined the Twilio WhatsApp Sandbox. Send 'join <keyword>' to +14155238886 from the recipient phone to connect.";
      } else if (data.code === 21608) {
        errorMsg = "Twilio Trial Error 21608: Recipient phone number is unverified in your Twilio Console.";
      }

      console.warn(`[WhatsApp Dispatch] Provider Rejection: ${deliveryLabel} — ${errorMsg}`);
      ...
      return {
        ...
        deliveryLabel,
        isRealDispatch: true,
        destination: shouldUseTestContact ? `${rawDestination} (Target: ${demoContact.verifiedPhone})` : toWhatsApp,
        routedToTestContact: shouldUseTestContact,
        actualDestination: toWhatsApp,
    ...
    console.error("[WhatsApp Adapter] Network exception calling Twilio:", err);
    return {
      channel: "WHATSAPP",
      provider: "TWILIO",
      deliveryMode: "FAILED",
      status: "FAILED",
      deliveryLabel: "WhatsApp Call Failed (Network Exception)",
      isRealDispatch: true,
